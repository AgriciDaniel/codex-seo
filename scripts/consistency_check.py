...
def check_runtime_invocations(texts):
    ...
    for f in carriers:
        ...
        for match in bare.finditer(content):
            # Codex SEO installs all deterministic scripts under this stable
            # default skill root. Unlike an arbitrary cwd-relative invocation,
            # this resolves to the installed isolated runtime.
            if "~/.codex/skills/seo/scripts/" in match.group(0):
                continue
...
def check_flow_lock(files):
    ...
    for rel, want in locked.items():
        ...
        if not os.path.exists(full):
            ...
            continue
        # The lock is generated from the repository's LF text. Normalize line
        # endings here so a Windows checkout cannot produce false mismatches.
