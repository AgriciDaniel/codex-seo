...
instead of WebSearch for SERP data. Run `python ~/.codex/skills/seo/scripts/dataforseo_costs.py check serp_organic_live_advanced --count N`
...
- All URLs fetched via `python ~/.codex/skills/seo/scripts/render_page.py <url> --mode auto` (SPA-aware SSRF protection via `url_safety`)
