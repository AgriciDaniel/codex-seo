...
Codex can invoke this skill from natural language. Commands: `/seo $1 $2` where `$1` is the command and `$2` is the URL or argument.

Scripts located at plugin root `scripts/` directory. Run via `codex-seo run <script.py>`. Plugin
...
| `codex-seo setup` | Explicitly create or refresh the isolated Python runtime and Chromium |
| `codex-seo doctor` | Check runtime readiness without changing the system |

## Runtime Setup

Run setup only when the user explicitly invokes `codex-seo setup` or explicitly asks
...
that setup is required, suggest `codex-seo setup` and do not improvise a `pip install`.
